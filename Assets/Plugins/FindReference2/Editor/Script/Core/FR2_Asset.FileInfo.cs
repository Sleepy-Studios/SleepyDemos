...
namespace vietlabs.fr2
{
    internal partial class FR2_Asset
    {
        ...
        internal void GuessAssetType()
        {
            ...
            if (REFERENCABLE_EXTENSIONS.Contains(ext))
            {
                bool isUnity = string.Equals(ext, ".unity", StringComparison.OrdinalIgnoreCase);
                type = isUnity ? AssetType.SCENE : AssetType.REFERENCABLE;

                if (string.Equals(ext, ".asset", StringComparison.OrdinalIgnoreCase) || isUnity || 
                    string.Equals(ext, ".spriteatlas", StringComparison.OrdinalIgnoreCase))
                {
                    if (!IsYamlFile(m_assetPath)) type = AssetType.BINARY_ASSET;
                }
                return;
            }
            
            if (REFERENCABLE_JSON.Contains(ext) || UI_TOOLKIT.Contains(ext))
            {
                type = AssetType.REFERENCABLE;
                return;
            }
            
            if (REFERENCABLE_META.Contains(ext))
            {
                type = AssetType.REFERENCABLE;
                return;
            }
            
            if (string.Equals(ext, ".fbx", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static readonly byte[] YamlHeader = { (byte)'%', (byte)'Y', (byte)'A', (byte)'M', (byte)'L' };
        ...
        private bool IsYamlFile(string path)
        {
            try
            {
                using (var stream = File.OpenRead(path))
                {
                    ...
                    if (stream.Read(buffer, 0, 5) < 5) return false;
                    
                    for (int i = 0; i < 5; i++)
                    {
                        if (buffer[i] != YamlHeader[i]) return false;
                    }
                    return true;
                }
            }
