...
namespace vietlabs.fr2
{
    ...
    partial class FR2_Cache
    {
        ...
        public static string GUIDToAssetPath(string guid)
        {
            ...
            if (!string.IsNullOrEmpty(cachedPath))
            {
                ...
                pathToGuidCache[cachedPath] = guid; // Bidirectional update
            }
        ...
        public static string AssetPathToGUID(string assetPath)
        {
            ...
            if (pathToGuidCache.TryGetValue(assetPath, out string cachedGuid))
            {
                return cachedGuid;
            }
            ...
            if (!string.IsNullOrEmpty(cachedGuid))
            {
                ...
                guidToPathCache[cachedGuid] = assetPath; // Bidirectional update
            }
        ...
        public static bool TryGetGUIDAndLocalFileIdentifier(UnityEngine.Object obj, out string guid, out long localId)
        {
            ...
            try
            {
                ...
                if (localIdProp != null)
                {
                    ...
                    guid = AssetPathToGUID(AssetDatabase.GetAssetPath(obj));
    ...
    internal partial class FR2_Cache : ScriptableObject
    {
        ...
        [NonSerialized] internal static readonly Dictionary<string, string> pathToGuidCache = new Dictionary<string, string>();
        ...
        internal static void Initialize()
        {
            ...
            if (readTS != null) readTS.Stop(); // in case FR2_Cache.asset being deleted mid-way
        ...
        internal static void ReadAssetContent()
        {
            ...
            {
                FR2_LOG.LogWarning($"{nameof(FR2_Cache)}::{nameof(ReadAssetContent)}() | Refresh content is already in progress at {readTS.currentIndex}");
                return;
            }
        ...
        private static void ReadAssetComplete()
        {
            ...
            FR2_LOG.LogWarning($"{nameof(FR2_Cache)}::{nameof(ReadAssetComplete)}() | LastContentRead: {_totalRead}");
        ...
        private static void BuildUsedByMap()
        {
            if (!MatchesStatus(nameof(BuildUsedByMap)
                    ...
                    , FR2_Status.RefreshDB // in case check but nothing is dirty
