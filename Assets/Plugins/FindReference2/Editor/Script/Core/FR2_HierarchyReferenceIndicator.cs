...
namespace vietlabs.fr2
{
    internal static class FR2_HierarchyReferenceIndicator
    {
        ...
        private static void Initialize()
        {
            ...
            EditorApplication.hierarchyChanged -= AutoRefreshReferenceCache;
            FR2_SceneCache.onReady -= AutoRefreshReferenceCache;
        ...
        private static void AutoRefreshReferenceCache()
        {
            if (FR2_SettingExt.disable) return;
            if (!FR2_SettingExt.isAutoRefreshEnabled) return;
            if (!FR2_Cache.isReady) return;
            
            FR2_LOG.Log($"{nameof(FR2_HierarchyReferenceIndicator)}::{nameof(AutoRefreshReferenceCache)}()");
