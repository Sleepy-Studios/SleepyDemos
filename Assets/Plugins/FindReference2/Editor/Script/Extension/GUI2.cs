...
namespace vietlabs.fr2
{
    ...
    internal static class GUI2
    {
        ...
        private static GUIStyle badgeNumberStyle
        {
            get
            {
                if (_badgeNumberStyle != null) return _badgeNumberStyle;
                _badgeNumberStyle = new GUIStyle(EditorStyles.miniLabel)
                {
                    alignment = TextAnchor.MiddleCenter,
                    fontSize = 8
                };
                return _badgeNumberStyle;
            }
        }

        public static void DrawBadge(Vector2 pos, int number, bool rowLayout)
        {
            if (number <= 0) return;

            var content = FR2_GUIContent.FromInt(number);
            var textSize = badgeNumberStyle.CalcSize(content);
            var badgeSize = Mathf.Max(16f, textSize.x + 8f);
            var badgeRect = new Rect(pos.x-2f, pos.y -1f, badgeSize + 4f, 18f);
            var tex = EditorGUIUtility.IconContent("sv_icon_dot0_pix16_gizmo").image;
            ...
            GUI.Label(badgeRect, content, badgeNumberStyle);
        }
    }
}