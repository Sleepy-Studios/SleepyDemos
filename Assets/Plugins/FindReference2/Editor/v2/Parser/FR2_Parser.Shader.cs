...
namespace vietlabs.fr2
{
    internal static partial class FR2_Parser // Shader
    {
        ...
        {
            FR2_LOG.Log($"[FR2] Reading shader file: {filePath}");
        ...
        private static (string guid, long fileId) ParseLine_Shader(string line)
        {
            ...
            {
                FR2_LOG.Log($"[FR2] Processing include line: {line.Trim()}");
            }
#endif
            ...
            if (line.TrimStart().StartsWith("#include"))
            {
                ...
                FR2_LOG.Log($"[FR2] Found quoted include path: '{includePath}'");
                if (!string.IsNullOrEmpty(includePath))
                {
                    string resolvedPath = ResolveShaderIncludePath(includePath);
                    FR2_LOG.Log($"[FR2] Resolved path: '{resolvedPath}'");
                    if (!string.IsNullOrEmpty(resolvedPath))
                    {
                        string guid = FR2_Cache.AssetPathToGUID(resolvedPath);
                        FR2_LOG.Log($"[FR2] Path to GUID: '{resolvedPath}' -> '{guid}'");
                        if (!string.IsNullOrEmpty(guid))
                        {
                            FR2_LOG.Log($"[FR2] SUCCESS - Shader Include: {includePath} -> {resolvedPath} -> {guid}");
                            return (guid, -1);
                        }
                    }
                }
            ...
            if (line.TrimStart().StartsWith("#include"))
            {
                ...
                if (!string.IsNullOrEmpty(includePath))
                {
                    ...
                    if (!string.IsNullOrEmpty(resolvedPath))
                    {
                        ...
                        if (!string.IsNullOrEmpty(guid))
                        {
                            FR2_LOG.Log($"[FR2] Shader Include: {includePath} -> {resolvedPath} -> {guid}");
                            return (guid, -1);
                        }
                    }
                }
        ...
        private static string ResolveShaderIncludePath(string includePath)
        {
            ...
            FR2_LOG.Log($"[FR2] Resolving shader include path: '{includePath}'");
            ...
            if (includePath.StartsWith("Packages/"))
            {
                ...
                {
                    FR2_LOG.Log($"[FR2] Found direct package path: {includePath}");
                    return includePath;
                }
            }
