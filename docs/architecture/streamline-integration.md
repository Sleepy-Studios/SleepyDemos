...
DLSS 是公共渲染模块，实验室仅提供观察场景。全局档位默认关闭并保存到 PlayerPrefs，场景不支持时保留用户偏好、单独显示实际回退状态。设置面板归 Hotfix 公共画质模块，不持有原生资源，不显示世界 RawImage。

URP 17.3 对带 Camera Stack 的相机禁用 TAA 抖动。公共接入因此在启用期间将玩法主相机输出到全尺寸离屏目标，使用原 Renderer 和后处理；运行时管线副本只在该世界相机开始渲染时使用 SDK 输入比例，DLSS 在后处理前恢复输出尺寸。独立公共呈现相机由运行时服务持久持有，以比例 1 输出后处理后的世界纹理，并承接原有 Overlay UI，不归导航中即将卸载的旧场景所有。玩法主相机的屏幕尺寸保持目标分辨率；关闭或切场景恢复原目标、管线及 UI 栈。不得把呈现相机作为 Camera.main 或新增 AudioListener。
