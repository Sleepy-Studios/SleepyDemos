...
- 新增或修改可稳定断言且有长期回归价值的业务规则、状态转换或公共契约时，优先补永久测试；适合自动化的行为优先先写失败测试，再实现最小代码。
- 文字、格式、视觉微调不机械新增测试脚本；视觉观感通过目标场景人工验证。临时调试探针、一次性可行性试验不默认保留为永久测试，运行验证也不等于必须新增测试文件。
- 清理测试时，删除重复覆盖和已完成的一次性迁移断言，不用源码字符串匹配锁定编辑器布局或局部写法。生成器的输出契约、当前明确的资源/场景契约和程序集依赖边界仍有回归价值，不因涉及资源或源码检查就一律删除。
- 优先扩展已有测试文件与两个现有测试程序集。临时验证脚本及其 `.meta`、临时资产在同一任务内清理，只清理本次创建的内容；保留永久测试时说明其长期回归价值。
