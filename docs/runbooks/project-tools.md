...
- 仓库当前没有旧 `UI Framework Validation` 菜单和验证 Prefab；基础 UI 自动化验证统一通过 Unity Test Runner 执行。
- `ViewTab` 模板使用 `UITab + ViewRoot/Parent` 结构；手风琴模板位于 `Assets/LoadResources/UI/Common/_TemplateInstantiatePrefab/Accordion/`。
