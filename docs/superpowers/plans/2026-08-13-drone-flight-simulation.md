...
- 最新恢复点：吊挂结构缩为一节连接杆，抓斗可见几何与 Collider 按 `0.7` 等比缩小；吊挂设备总质量进入普通设置，默认 `0.05 kg`，运行时按配置重新分配到各 Rigidbody。等待用户验证尺寸、摆动与抓取范围。
...
当前恢复点：完成自动化验证后暂停，等待第二轮实际试飞反馈；不进入 Blender 成品模型或无人机钓鱼。
...
### 第二轮试玩补丁：载荷受力时机与短卷扬锚点

- [x] 抓取弱约束建立后继续单独报告物体真实质量，但悬停前馈只按 PhysX 约束沿重力方向的受力平滑计入；载荷仍由地面支撑时不瞬间加入完整质量。
...
- [ ] 用户手动确认当前 `0.05 m` 短卷扬下，0.75 kg 物体可离地、F3 受力载荷渐进变化且释放后有自然高度响应。
